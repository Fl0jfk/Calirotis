...
function Meats(){
    const data = useData();
    return (
        <section className="flex flex-col w-full max-w-[1050px] p-4 items-center justify-center gap-6" id="meats">
            ...
            <div className="flex sm:flex-col gap-4 w-full items-center">
                {data.meats.map((meat:{id:number, name:string, halal:boolean, description:string, image:string}) => (
                    <div key={meat.id} className="flex rounded-xl flex-col w-full" style={{backgroundImage: `url(${meat.image})`, backgroundSize: "cover", backgroundPosition: "50%, 50%"}}>
                        <div className="flex flex-col justify-between bg-gray-900 opacity-80 gap-8 p-4 w-full h-[600px] sm:h-[520px] rounded-xl hover:opacity-0 cursor-pointer">
            ...
            <div className="flex flex-col self-center rounded-xl w-full h-full max-w-[600px]" style={{backgroundImage: "url(https://calirotis.s3.eu-west-3.amazonaws.com/assets/img/accompagnement.webp)", backgroundSize: "cover", backgroundPosition: "50%, 50%"}}>
